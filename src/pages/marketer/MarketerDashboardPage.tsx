...
export default function MarketerDashboardPage() {
  ...
  return <div><PageHeader title={`Welcome, ${user?.firstName ?? 'Marketer'}`} subtitle="Your sales, KPI and commission overview" />
    ...
    <Card noPadding><Card.Header className="p-5 pb-0" title="Recent sales" action={<Link to="/marketer/sales" className="text-sm text-[#1565C0]">View all</Link>} />{sales.mySales.slice(0, 5).map(s => <div key={s.id} className="flex items-center justify-between gap-3 border-t border-[#EEEEEE] px-5 py-3"><div><p className="font-medium text-[#212121]">{s.customerName}</p><p className="text-xs text-[#757575]">{s.productName} · {formatDate(s.saleDate)}</p></div><div className="text-right"><p className="text-sm font-medium">{formatCurrency(s.amount)}</p><SaleStatusBadge status={s.status}/></div></div>)}{sales.mySales.length === 0 && <p className="p-5 text-sm text-[#757575]">No sales have been recorded yet.</p>}</Card>
  </div>
}
